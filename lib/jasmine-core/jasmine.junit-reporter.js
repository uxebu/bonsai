(function() {
    ...
    JUnitXmlReporter.prototype = {
        ...
        reportSpecResults: function(spec) {
            ...
            spec.output = '<testcase classname="{{TAG}}.' +
            ...
            for (var i = 0; i < resultItems.length; i++) {
               var result = resultItems[i];
                if (result.type == 'expect' && result.passed && !result.passed()) {
                    var failure = trim(escapeInvalidXmlChars(result.message));
                    try {
                        var stack = trim(escapeInvalidXmlChars(result.trace.stack));
                    } catch(e) {
                        var stack = 'No stack trace available (PhantomJS does not support stack traces)';
                    }
                    spec.output += '<failure message="' + failure + '">' + stack + '</failure>';
               }
            }
        ...
        reportSuiteResults: function(suite) {
            ...
            var suiteName = this.getFullName(suite, true);
            suite.output = '\n<testsuite name="{{TAG}}.' + suiteName +
                '" errors="0" tests="' + specs.length + '" failures="' +
                failedCount + '" time="' + suite.duration + '" timestamp="' +
                ISODateString(suite.startTime) + '"><properties><property name="foo" value="bar"/></properties>';
        ...
        reportRunnerResults: function(runner) {
            ...
            for (var i = 0; i < suites.length; i++) {
                var suite = suites[i];
                var fileName = this.getFullName(suite, true);
            ...
            if(typeof window != 'undefined') window.testsComplete = true;
        },
        ...
        writeFile: function(filename, text) {
            ...
            try {
                phantom.saveToFile(text, filename + ".xml");
                return;
            } catch (f) {
            }
            try {
                var fs = require("fs");
                var fd = fs.openSync(filename + ".xml", "w");
                fs.writeSync(fd, text, 0);
                fs.closeSync(fd);
                return;
            } catch (g) {}
            try {
                window.JUnitTestResults = window.JUnitTestResults || [];
                window.JUnitTestResults.push({
                    filename: filename,
                    text: text
                });
            } catch (h) {}
